...
var logSummaryCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		fmt.Println("\nBy credential")
		printTopNSimple(credCounts, 5)

		// By domain breakdown
		fmt.Println("\nBy domain")
		printTopNSimple(domainCounts, 5)
...
func exportCSV(writer *os.File, logs []proxy.AuditEvent) error {
	...
	// Write headers
	headers := []string{"id", "timestamp", "agent_id", "identity_level", "method", "target_url", "domain", "status_code", "duration_ms", "status", "reason", "redacted", "secret_keys", "auth_styles"}
...
// printTopNSimple prints the top N entries from a counts map.
func printTopNSimple(counts map[string]int, n int) {
	type entry struct {
		name  string
		count int
	}
...
func runLogList(cmd *cobra.Command, args []string) error {
	...
	if useTail {
		...
		for {
			tailFilter := log.Filter{
				Since: lastSeen,
				Limit: 20,
			}
			logs, err := logService.QueryLocal(tailFilter)
			if err == nil && len(logs) > 0 {
				for i := len(logs) - 1; i >= 0; i-- {
					l := logs[i]
					if seenIDs[l.ID] {
	...
	for {
		...
		headers := []string{"#", "TIME", "AGENT", "IDENTITY", "CREDENTIAL", "DOMAIN", "STATUS", "DUR"}
		rows := [][]string{}
		...
		for i, l := range logs {
			...
			rows = append(rows, []string{idx, t, ag, ident, cred, domain, status, dur})
		}
...
func displayLogBasic(l proxy.AuditEvent) {
	...
	fmt.Printf("%s  %s  →  %s  %s %s  %s  %s\n", t, ag, l.Domain, strings.ToUpper(l.Method), path, status, dur)
}
